...
volume = function(h,hmax,ftn) {
  #we have function 'ftn' as the cross-sectional area of at height 'h', with 'hmax' as the max height
  if (h < 0) {
    return(NA)
  } else if (h > hmax) {
    return(NA)
...
height = function(h,hmax,v,ftn) {
  #our function for computing the new dam level from a given current height 'h', the maximum dam level 'hmax',
