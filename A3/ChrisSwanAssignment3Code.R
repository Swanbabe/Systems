...
tanksim = function(num_years, maxraintank, maxgreytank, phat, mhat, lahat, plotflag = F) {
  ...
  for (year in 1:num_years) {
    ...
    for (i in 1:365) {
      ...
      greytank[i] = greytank[i] + (numshower * showersize) + (numwash() * washsize)
      ...
      if (gardenreq*gardenarea <= raintank[i]) raintank[i] = raintank[i] - gardenarea*gardenreq
...
tanksim(2, 3000, 1000, p_est, m_est, lam_est, T)


ggplot() + geom_line(aes(days, tank_rain_av), colour = 'blue', xlab("Days"))


vec1 = c(1,2,35,5)
vec2 = c(5,4,6,9)
data.frame(vec1, rbind(vec2, vec2, vec2, vec2))
